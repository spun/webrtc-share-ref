...
sealed class ModuleInstallException : Exception() {
    /**
     * An error occurred while launching the installation request.
     */
    class RequestFailed : ModuleInstallException()

    /**
     * An error occurred during the installation of the module.
     */
    class InstallFailed : ModuleInstallException()
}

/**
 * Starts an urgent installation request and emits the progress percentage
 * notified by the [ModuleInstallClient] listener.
 */
fun ModuleInstallClient.installFlow(
    ...
    // services will not be aware (the call to installModules(...).await does not suspend
    // for the whole installation process), and it will keep installing the module.
    //
